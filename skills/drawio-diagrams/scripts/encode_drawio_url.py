...
The diagram XML is carried in the URL fragment (after `#`). The fragment is
not part of the initial HTTP request, but JavaScript served by the remote
diagrams.net origin can read it. Do not use this fallback for confidential,
private, regulated, credential-bearing, or otherwise sensitive diagrams.
...
if __name__ == "__main__":
    ...
    if len(args) != 1:
        ...
        sys.exit(2)
    print(
        "warning: diagrams.net page JavaScript can read the URL fragment; "
        "do not use browser fallback for sensitive diagrams",
        file=sys.stderr,
    )
