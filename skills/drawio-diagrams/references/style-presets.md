...
Only user presets can have `"default": true`. When the user says *"make `<built-in-name>` my default"*, copy the built-in JSON to `~/.drawio-diagrams/styles/<name>.json` first, then set `default: true` on the copy — leave the shipped built-in untouched.

**Name normalisation and path safety:**

1. Lowercase the user-provided name.
2. Require the entire normalized name to match `^[a-z0-9][a-z0-9_-]{0,63}$`. Reject names containing dots, slashes, whitespace, shell metacharacters, or any other character; ask the user for a safe short name.
3. Resolve the target against the intended root before every read, write, copy, delete, or rename. The resolved target's parent must equal the resolved root (`~/.drawio-diagrams/styles/` for user presets or `<this-skill-dir>/styles/built-in/` for built-ins). Stop if it does not.
4. Never interpolate a preset name or path into a shell command. Use file APIs for file operations and an argument array for subprocesses. If a shell is unavoidable, quote each already-validated path as a separate argument.
...
3. **Validate the name and build the candidate.** Apply the safe-slug and resolved-parent rules above before any file operation. Create a dedicated temporary directory with the platform's secure tempfile API, then write `candidate.json` and `sample.drawio` inside it. Do **not** save to the user preset directory yet.
4. **Render a sample** using the sample-diagram skeleton in `references/style-extraction.md`, parameterized by the candidate preset. Export PNG to `./preset-<name>-sample.png`, where `<name>` is the validated safe slug. Invoke draw.io with a subprocess argument array, then run `repair_png.py` on the exact output path (see the Rendering the sample steps in `style-extraction.md`).
5. **Show the user:**
...
6. **Wait for approval:**
   - "save" / "looks good" → resolve and verify the user preset target, then write it with file APIs. Create `~/.drawio-diagrams/styles/` if it doesn't exist. Delete only the dedicated temporary directory and the exact sample PNG created by this flow.
...
| Preset name fails the safe-slug rule | Stop before any file access; ask for a name containing only lowercase letters, digits, `_`, or `-`. |
...
*Apply the safe-slug and resolved-parent rules to all `<name>`, `<a>`, and `<b>` arguments before any file operation. Use file APIs; never construct `rm`, `mv`, `cp`, or other shell commands from user-provided names.*

| User says | Agent does |
|---|---|
| "list my styles", "what styles do I have", "show me my style presets" | Read `~/.drawio-diagrams/styles/` and `<this-skill-dir>/styles/built-in/`. Print a table: `name`, `location` (user/built-in), `source.type`, `confidence`, `default` flag. Built-ins shadowed by a user preset are marked so. |
| "show my `<name>` style", "what's in `<name>`" | Print the preset JSON (pretty-printed) + a one-line summary (source, confidence, is-default). |
| "make `<name>` the default", "set `<name>` as default" | If `<name>` is a user preset: set `default: true` on it; clear `default` on any other user preset that had it; save both files. If `<name>` is a built-in: copy `<this-skill-dir>/styles/built-in/<name>.json` → `~/.drawio-diagrams/styles/<name>.json` first, then set `default: true` on the copy. Never mutate the shipped built-in. |
| "remove default", "unset default" | Clear `default: true` from whichever user preset has it. |
| "delete `<name>`", "remove `<name>`" | Confirm first. Resolve the validated user-preset target, verify its parent is the user styles root, then delete that exact file with a file API. Refuse to delete files under `<this-skill-dir>/styles/built-in/` — suggest shadowing with a user preset of the same name. |
| "rename `<a>` to `<b>`" | Resolve and verify both validated user-preset targets, fail if the destination exists, rename with a file API, then update the `name` field inside. Fails if `<a>` is a built-in (offer to copy-then-rename instead). |
