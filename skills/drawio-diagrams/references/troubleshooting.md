...
| Linux: draw.io refuses to start as root | Do not disable the Chromium sandbox. Run draw.io as an unprivileged user in a restricted container/VM, or deliver `.drawio` XML for local export. |
| Linux: `Failed to get 'appData' path` / `Home directory not accessible` | As an unprivileged user, create an isolated directory with `mktemp -d`, set `HOME` only for the draw.io process, then remove that exact temporary directory after export (drawio-desktop#127). |
...
**Open a browser-fallback URL for a non-sensitive diagram only.** The remote
diagrams.net page can read the full fragment. `cmd.exe /c start` strips the URL fragment
