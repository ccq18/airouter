...
Precondition: `references/style-presets.md` has already normalized and validated `<name>` as a safe slug and established the resolved target roots. Never use an unvalidated name in a path or shell command.

1. Create a dedicated temporary directory with the platform's secure tempfile API and write the filled XML to `sample.drawio` inside it.
2. Save the PNG as `./preset-<name>-sample.png` in the user's working directory. Resolve the parent directory and use the validated slug only.
3. Run draw.io with an argument array equivalent to `drawio -x -f png -e -s 2 -o <sample-png> <temporary-drawio>` (substitute the binary name resolved in SKILL.md Step 1). Do not build a shell command string.
4. Repair the IEND chunk by invoking `python3 <this-skill-dir>/scripts/repair_png.py <sample-png>` as an argument array — the `-e` flag truncates the PNG the same way the main workflow's step 7 does, so the sample needs the same fix to be readable.
5. Show the user: preset summary table + PNG path + provenance/confidence line.

### Approval loop

- "save" / "looks good" → resolve and verify the validated target under `~/.drawio-diagrams/styles/`, write it with a file API, then delete only the dedicated temporary directory and the exact sample PNG.
