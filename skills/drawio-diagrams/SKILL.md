...
description: Use when a user requests architecture diagrams, flowcharts, swimlanes, sequence diagrams, ERDs, UML, C4, network or cloud topology, diagrams from code or infrastructure, Mermaid or PlantUML, editable .drawio files, or PNG/SVG/PDF/JPG diagram exports.
license: MIT
metadata: {"homepage":"https://github.com/Agents365-ai/drawio-skill","compatibility":"Requires Python 3. The draw.io desktop CLI is optional and only needed for local PNG/SVG/PDF/JPG export; XML and browser fallback work without it. Optional auto-layout needs Graphviz dot. Vision self-check is skipped when unavailable.","platforms":["macos","linux","windows"],"openclaw":{"emoji":"📐","os":["darwin","linux","win32"],"install":[{"id":"brew-drawio","kind":"brew","formula":"drawio","bins":["drawio"],"label":"Install draw.io for local exports","os":["darwin"],"optional":true},{"id":"brew-graphviz","kind":"brew","formula":"graphviz","bins":["dot"],"label":"Install Graphviz for optional autolayout.py","os":["darwin"],"optional":true}]},"hermes":{"tags":["drawio","diagram","flowchart","architecture","visualization","uml"],"category":"design","requires_tools":["python3"],"related_skills":["mermaid","excalidraw","plantuml"]},"author":"Agents365-ai","version":"1.28.0"}
---
...
Create structured diagrams with one self-contained skill: choose the appropriate source format, generate and validate editable `.drawio` XML, extract graphs from code/SQL/OpenAPI/IaC, apply automatic layout and styles, and export to PNG/SVG/PDF/JPG using the native draw.io desktop CLI when available.
...
This directory merges the diagram-selection and authoring guidance previously kept in `drawio-diagrams` with the complete implementation toolkit previously kept in `drawio-skill`. Do not delegate to or require a second draw.io skill.

## Core Principles

- Correct meaning comes before visual polish.
- Keep one abstraction level per diagram.
- Use the same shape, size, color and label pattern for the same semantic role.
- Keep connectors away from nodes, prefer orthogonal routing, and add a legend when colors or line styles carry meaning.
- Derive factual diagrams from code, configuration, DDL, OpenAPI, Terraform, Kubernetes or user-provided evidence instead of inventing system structure.

## Choose the Artifact

| Need | Default |
|---|---|
| Editable delivery, formal documentation, complex layout, rich shapes | `.drawio` |
| README, PR or Markdown-native lightweight diagram | Mermaid |
| Text-owned UML sequence, component or class diagram | PlantUML |
| Software architecture levels | C4 / Structurizr DSL or C4-generated `.drawio` |
| Whiteboard-style discussion draft | Excalidraw |
| Fast explanation without a requested file | Mermaid, unless layout or styling requires `.drawio` |

The bundled scripts focus on `.drawio` generation, import, layout, validation, conversion and export. Mermaid, PlantUML, Structurizr and Excalidraw remain valid source formats when they better match the requested deliverable.
...
Use a lighter artifact from the table above when the user needs Markdown-native diagram-as-code or a discussion sketch rather than a rich `.drawio` deliverable.
...
| File | Read it when |
|---|---|
| `references/source-best-practices.md` | You need source-backed rules for choosing draw.io, Mermaid, PlantUML, C4/Structurizr or Excalidraw and for keeping diagrams concise, clear and consistent |
| `references/drawio-format.md` | You need the minimal uncompressed `.drawio` XML skeleton or a compact structural checklist |
...
- Else, check `~/.drawio-diagrams/styles/` for any file with `"default": true`. If found → active preset = that one.
- Else → no preset active; fall through to the built-in color/shape/edge conventions for the rest of the workflow.

Before using a named preset, lowercase it and require a full match of `^[a-z0-9][a-z0-9_-]{0,63}$`; reject anything else before file access. Resolve the candidate under the intended user or built-in styles root and verify its resolved parent is exactly that root. Never interpolate a preset name into a shell command. Read `references/style-presets.md` for the full file-safety rules.

Load the preset JSON from `~/.drawio-diagrams/styles/<name>.json`, falling back to `<this-skill-dir>/styles/built-in/<name>.json`. If the named preset exists in neither location, tell the user the name is unknown, list the available presets (user dir + built-in), and stop — do **not** silently fall back to defaults.
...
1. `~/.drawio-diagrams/styles/<name>.json` — user presets (survive `git pull`)
...
# Linux (headless — run as an unprivileged user; use an isolated temporary HOME)
DRAWIO_HOME="$(mktemp -d)"
HOME="$DRAWIO_HOME" xvfb-run -a --server-args="-screen 0 1280x1024x24" \
  drawio -x -f png -e -s 2 -o diagram.drawio.png input.drawio --disable-gpu
rm -rf "$DRAWIO_HOME"
# Do not run draw.io as root and do not disable the Chromium sandbox.
# Use an unprivileged user in a restricted container/VM; otherwise deliver XML for local export.
...
Default prints a `https://viewer.diagrams.net/...#R…` viewer URL; `--edit` prints a `https://app.diagrams.net/...#create=…` URL that opens straight into the editable editor. The diagram XML is `encodeURIComponent`-encoded, deflate-compressed, and base64'd into the URL fragment. Browsers omit the fragment from the initial HTTP request, but JavaScript served by the remote diagrams.net origin can read it. **Do not use browser fallback for confidential, private, regulated, credential-bearing, or otherwise sensitive diagrams.** For those diagrams, deliver `.drawio` XML for a trusted local desktop workflow or generate a self-contained local HTML artifact where appropriate. The `encodeURIComponent` step is mandatory: without it, any diagram containing a literal `%` or non-ASCII (e.g. CJK) label makes the browser throw "URI malformed" and the diagram never opens.
...
| Scenario | Behavior |
|----------|----------|
| draw.io CLI missing, Python available, diagram is non-sensitive | Use browser fallback (diagrams.net URL) |
| draw.io CLI missing and diagram is sensitive | Do not create or open a remote-origin browser URL; deliver `.drawio` XML for a trusted local desktop workflow or a self-contained local artifact |
...
| Export fails on Linux server (headless) | Run as an unprivileged user in a restricted container/VM with the Chromium sandbox enabled; try `xvfb-run -a`, `--disable-gpu`, a writable non-shared `HOME`, and required desktop libraries. If export still fails, deliver `.drawio` XML for local export. Never retry as root with `--no-sandbox`. |
