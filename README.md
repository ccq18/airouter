...
Airouter 在需要重新选择上游时，按管理页里的配置顺序选择可用上游。越靠前，优先级越高；可以在管理页用“上移”调整顺序。

大致规则是：

- 当前活动配置可用且支持当前接口时，会继续使用当前配置。
- 当前配置不可用或不支持当前接口时，会按配置顺序寻找第一个可用配置；单纯上移不会触发当前配置切换。
...
你不需要手动记住每个账号状态，管理页会展示当前运行态，也可以手动切换、上移或删除配置。
