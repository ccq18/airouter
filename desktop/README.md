...
The tag workflow builds macOS arm64, macOS x64, and Windows x64 artifacts. It publishes installer assets only, matching the stable release flow used before updater packaging was introduced. The expected release assets are:
...
Signed updater packages and `latest.json` are not generated by the tag workflow. To create them from already collected signed updater artifacts, run:
