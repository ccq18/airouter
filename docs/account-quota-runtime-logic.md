...
- 每个 apikey 保存最近 100 个已完成真实请求结果并计算错误率；切换时选择错误率最低的可用候选，无样本按 0%，同率保持配置顺序
...
当真实业务请求已经命中某个 token 账号，且响应被识别为额度错误或上游错误时，系统会直接把当前账号标记为不可用。请求非 `gpt-5.4-mini` 或它的日期版本后缀时，成功响应被降级成 `gpt-5.4-mini` 或同名日期版本只触发本次请求切走，不标记账号不可用；系统会在运行态模型观测里记录 `downgraded = true`。apikey 账号记录最近 100 个已完成真实请求结果，并用失败数除以样本数得到错误率。apikey 的业务错误只触发请求级切换，不会把当前配置标记为不可用；切换时选择错误率最低的可用候选，没有候选时继续使用当前配置。
...
- `apikey` 直连上游保存最近 100 个真实请求的成功/失败结果，用于计算候选错误率，并始终保留为可用配置
...
- apikey 请求结果只更新最近 100 个请求的错误率，不进入不可用标记流程
- 设置对应失败原因
- 记录 `lastError`
...
- fallback apikey 失败后，会在同链路剩余可用候选中选择最近 100 个请求错误率最低者，不摘除当前 apikey

1. 如果当前活动配置符合当前路由能力且 `runtime.available = true`，继续保持当前配置。
2. 如果当前活动配置不可用、不支持当前路由或被本次 failover 排除，token 按原调度规则选择；apikey 按最近 100 个请求错误率从低到高选择。
3. apikey 无历史样本时错误率为 0%；错误率相同时按 `configs[]` 顺序选择。
4. 如果没有找到任何可用账号：
   - 保留当前账号
   - apikey 即使错误率较高也不摘除，继续作为当前能力池的最后可用配置
