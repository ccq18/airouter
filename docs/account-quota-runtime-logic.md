...
- 直连上游在响应提交给客户端前遇到任意非 200 HTTP 状态、请求失败、响应体中断，或 `/v1/responses` HTTP 200 JSON/SSE 中可识别的使用上限错误时，会在本次请求内先尝试切到下一个可用配置
- 普通 apikey 代理在响应提交给客户端时记录成功；响应提交后发生的传输中断不会再触发透明切换，也不会把本次请求记为失败
- 最近 5 分钟内最多 10 个已完成真实请求累计 3 次失败时，表示 apikey 错误频率达到切换阈值；业务错误只触发请求级切换，不会把 apikey 标记为不可用
...
当真实业务请求已经命中某个 token 账号，且响应被识别为额度错误或上游错误时，系统会直接把当前账号标记为不可用。请求非 `gpt-5.4-mini` 或它的日期版本后缀时，成功响应被降级成 `gpt-5.4-mini` 或同名日期版本只触发本次请求切走，不标记账号不可用；系统会在运行态模型观测里记录 `downgraded = true`。apikey 账号记录最近 5 分钟内最多 10 个已完成真实请求结果；响应提交给客户端前的非 200 HTTP 状态、请求失败、响应体中断，或 `/v1/responses` HTTP 200 JSON/SSE 中可识别的使用上限错误累计达到 3 次时，表示错误频率达到切换阈值。apikey 的业务错误只触发请求级切换，不会把当前配置标记为不可用；没有候选时继续使用当前配置。
...
- `apikey` 直连上游最近 5 分钟内最多 10 个真实请求累计 3 次提交响应前的非 200 HTTP 状态、普通代理请求失败或响应体中断时，判定达到切换阈值，但始终保留为可用配置
...
- apikey 达到窗口阈值后只记录切换建议，不进入不可用标记流程
- 设置对应失败原因
- 记录 `lastError`
...
- fallback apikey 失败后，会尝试同链路下一个 token 或 fallback 配置；最近 5 分钟窗口累计 3 次失败只表示达到切换阈值，不摘除当前 apikey
...
4. 如果没有找到任何可用账号：
   - 保留当前账号
   - apikey 即使失败窗口已达到阈值也不摘除，继续作为当前能力池的最后可用配置
