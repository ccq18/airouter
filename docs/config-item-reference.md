...
- `apikey` 直连上游在响应提交给客户端前遇到任意非 200 HTTP 状态、请求失败、响应体中断，或 `/v1/responses` HTTP 200 JSON/SSE 中可识别的使用上限错误时，会在本次请求内排除当前配置并尝试切到下一个可用配置；没有下一候选时继续使用当前配置。最近 5 分钟内最多保存 10 个已完成真实请求结果，累计 3 次失败表示达到切换阈值，但不会摘除 apikey 或把 `runtime.available` 改为 `false`
...
- `apikey` 配置项在响应提交给客户端前遇到任意非 200 HTTP 状态、请求失败、响应体中断，或 `/v1/responses` HTTP 200 JSON/SSE 中可识别的使用上限错误时，会在本次请求内排除当前配置并尝试切到下一个可用配置。如果没有可切换配置，则继续使用当前配置并透传当前上游错误。响应已经开始写给客户端后不再透明切换，也不会因为后续传输中断把本次请求记为失败。最近 5 分钟内最多 10 个已完成真实请求累计 3 次失败时只表示达到切换阈值，不会临时标记 apikey 为不可用
